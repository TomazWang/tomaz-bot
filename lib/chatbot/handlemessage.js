...
function handleTextMessage(botName, chatMessage) {
  ...
  logger.debug(`handlemessage.js#handleTextMessage: rawText = ${rawText}`);
  logger.debug(`handlemessage.js#handleTextMessage: source = ${JSON.stringify(chatMessage.source)}`);

  if (chatMessage.source.type !== 'user' && !rawText.startsWith(botName)) {
  ...
  let realContext = rawText.substr(rawText.indexOf(botName) + botName.length).trim();
