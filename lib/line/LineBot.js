...
} from '../chatbot/messages/ResponseMessage';
...
const { createConfirm, createButton, createThumbnail } = require('../line/lineTempButton');
const { createMessageAction, createPostbackAction, createUriAction } = require(
  '../line/lineAction');
...
module.exports = class LineBot {
  ...
  static convertActionToLineAction(action) {
    const {
      label, type, actionName, datas, text,
    } = action;

    if (type === 'post') {
      return createPostbackAction(label, text, actionName, datas);
  ...
  /**
   * @param resMessage {ResponseMessage} ResponseMessage with type = RES_MSG_TYPE_BUTTON
   * @return a line template message json.
   */
  static convertResMsgToBtnTemp(resMessage) {
    ...
    const lineActions = actions.map(ac => this.convertActionToLineAction(ac));
  ...
  /**
   * @param resMessage {ResponseMessage} a ResponseMessage with type = RES_MSG_TYPE_CONFIRM
   * @return a line template message json.
   */
  static convertResMsgToConfirmTemp(resMessage) {
    const {
      message, actions,
    } = resMessage.context;

    const lineActions = actions.map(ac => this.convertActionToLineAction(ac));
    const confirmTemp = createConfirm(message, lineActions);

    return createTempMsg(message, confirmTemp);
  }
  ...
  processLineEvent(event) {
    ...
    logger.debug(
      `linebot.js#processLineEvent: source = ${JSON.stringify(source)}`);
  ...
  /**
   * @param resMessage {ResponseMessage} a ResponseMessage.
  ...
  static convertToLineMessage(resMessage) {
    ...
    switch (type) {
      ...
      case RES_MSG_TYPE_BUTTON:
        // button template
        return LineBot.convertResMsgToBtnTemp(resMessage);
      case RES_MSG_TYPE_CONFIRM:
        return LineBot.convertResMsgToConfirmTemp(resMessage);
      default:
        return {};
    }
  }
  ...
  processMessageEvent(source, messageEvent) {
    ...
    if (messageType === 'text') {
      ...
      handleMessage(this.name, chatMsg).then((action) => {
        ...
        logger.error(
          `linebot.js#_processMessageEvent: action ${action} no regonized`);
  ...
  pushLineMessage(receiverId, lineMessages) {
    ...
    return new Promise((resolve, reject) => {
      ...
      logger.debug(
        `linebot.js#_pushLineMessage: reqData = ${JSON.stringify(reqData)}`);
      ...
      request.post(
        ...
        (error, response, body) => {
          ...
          if (statusCode !== 200) {
            logger.error(
              `pushLineMessage: Error status code while sending message (code = ${statusCode})`);
            const { message: msg, details } = resBody;
            reject(new Error(
              `Push line message error with code ${statusCode}, ${msg}\n${JSON.stringify(
                details)}`));
            return;
          }
  ...
  pushMessage(responseMessage) {
    ...
    return Promise.resolve(responseMessage).then(rm => this.pushLineMessage(
      rm.des.to,
      [ LineBot.convertToLineMessage(rm) ],
    )).then(() => sleep(this.sendMessageInterval)).catch((err) => {
      // handle error later
